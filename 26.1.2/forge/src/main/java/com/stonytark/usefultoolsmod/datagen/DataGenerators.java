...
public class DataGenerators {
    ...
    public static void onGatherData(GatherDataEvent event) {
        ...
        generator.addProvider(event.includeClient(),
                new NamespaceFilteringProvider("Useful Tools Block Model Definitions",
                        new ModBlockStateProvider(packOutput, existingFileHelper)));
        generator.addProvider(event.includeClient(),
                new NamespaceFilteringProvider("Useful Tools Item Model Definitions",
    ...
    /**
     * Vanilla's 26.x {@code ModelProvider} always emits the complete vanilla model
     * set and its final {@code getName()} returns the same value for both instances.
     * Keep only this mod's namespace and give the deliberately split providers stable,
     * unique identities. This prevents shipping thousands of replacement Minecraft
     * assets while retaining vanilla's model validation machinery.
     */
    private record NamespaceFilteringProvider(String getName, DataProvider delegate) implements DataProvider {
        @Override
        public CompletableFuture<?> run(CachedOutput output) {
            return delegate.run((path, data, hash) -> {
                for (int index = 0; index + 1 < path.getNameCount(); index++) {
                    if ("assets".equals(path.getName(index).toString())
                            && UsefultoolsMod.MOD_ID.equals(path.getName(index + 1).toString())) {

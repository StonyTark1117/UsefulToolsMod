...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    private void blockWithItem(RegistryObject<Block> blockDeferredHolder) {
        simpleBlockWithItem(blockDeferredHolder.get(), cubeAll(blockDeferredHolder.get()));
    }

    private void blockItem(RegistryObject<? extends Block> blockDeferredHolder) {
        simpleBlockItem(blockDeferredHolder.get(), new ModelFile.UncheckedModelFile("tutorialmod:block/" +
                ForgeRegistries.BLOCKS.getKey(blockDeferredHolder.get()).getPath()));
    }

    private void blockItem(RegistryObject<? extends Block> blockDeferredHolder, String appendix) {
        simpleBlockItem(blockDeferredHolder.get(), new ModelFile.UncheckedModelFile("tutorialmod:block/" +
                ForgeRegistries.BLOCKS.getKey(blockDeferredHolder.get()).getPath() + appendix));
    }
}
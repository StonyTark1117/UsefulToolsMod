 ...
 * underlying {@link ModConfigSpec.ConfigValue} happens when the user clicks
 * "Done", which then runs {@link ModConfigSpec#save()}; "Cancel" discards
 * the staged edits.
 *
 * <p>Registered as a NeoForge config screen via
...
public class UsefulToolsConfigScreen extends Screen {
    ...
    private void populate(OptionList list) {
        ...
        for (UnmodifiableConfig.Entry e : root.entrySet()) {
            String name = e.getKey();
            Object val = e.getValue();
            if (val instanceof UnmodifiableConfig section) {
                list.addHeader(Component.literal("§e" + prettify(name)));
                addSection(list, name, section);
            } else if (val instanceof ModConfigSpec.ConfigValue<?> cv) {
                list.addRow(makeRow(cv, canonicalKey(name)));
            }
        }
    }

    private void addSection(OptionList list, String sectionPath, UnmodifiableConfig section) {
        for (UnmodifiableConfig.Entry e : section.entrySet()) {
            String key = e.getKey();
    ...
    private RowEntry makeRow(ModConfigSpec.ConfigValue<?> cv, String label) {
        ConfigDescriptors.Descriptor descriptor = ConfigDescriptors.get(label);
        ...
        if (value instanceof Boolean b) {
            return new BoolRow(this, name, tooltip, b, newVal -> stageWrite(cv, newVal));
        } else if (value instanceof Integer
                || value instanceof Long
                || value instanceof Double) {
            return new NumberRow(this, name, tooltip, String.valueOf(value), text -> stageNumberWrite(cv, value, text));
        } else {
    ...
    private void stageWrite(ModConfigSpec.ConfigValue<?> cv, boolean v) {
        pendingWrites.add(() -> ((ModConfigSpec.ConfigValue<Boolean>) cv).set(v));
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    private void stageNumberWrite(ModConfigSpec.ConfigValue<?> cv, Object original, String text) {
        try {
            Object parsed;
            ...
            pendingWrites.add(() -> ((ModConfigSpec.ConfigValue) cv).set(parsed));

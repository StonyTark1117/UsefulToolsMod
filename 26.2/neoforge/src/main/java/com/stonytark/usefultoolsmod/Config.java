...
public class Config {
    ...
    private static final ModConfigSpec.BooleanValue REMOTE_DETONATION_ENABLED;
    private static final ModConfigSpec.BooleanValue CONTROLLED_DROPS;
    private static final ModConfigSpec.DoubleValue CONTROLLED_ENTITY_DAMAGE;
    private static final ModConfigSpec.IntValue REMOTE_RANGE;
    ...
    static {
        ...
        WRAITH_SPAWN_CHANCE = BUILDER.comment("Fraction of eligible Wraith spawn attempts.").defineInRange("wraithSpawnChance", 0.0375, 0.0, 1.0);
        ...
        CONTROLLED_DROPS = BUILDER.comment("Preserve normal block drops from controlled explosives.").define("preserveDrops", true);
        CONTROLLED_ENTITY_DAMAGE = BUILDER.comment("Maximum controlled-explosion entity damage.").defineInRange("entityDamageCap", 8.0, 0.0, 8.0);
        REMOTE_RANGE = BUILDER.comment("Maximum remote detonation range.").defineInRange("remoteRange", 128, 16, 1024);
        BUILDER.pop();
    ...
    static void onLoad(final ModConfigEvent event) {
        if (!(event instanceof ModConfigEvent.Loading)
                && !(event instanceof ModConfigEvent.Reloading) || !SPEC.isLoaded()) {
            return;
        }
        // Set enabled

...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    private void trimmedArmorItem(DeferredHolder<Item, ? extends Item> itemDeferredHolder) {
        ...
        if(itemDeferredHolder.get() instanceof ArmorItem armorItem) {
            trimMaterials.forEach((trimMaterial, value) -> {
                ...
                String armorItemPath = itemDeferredHolder.getId().toString();
                ...
                this.withExistingParent(itemDeferredHolder.getId().getPath(),
                        ...
                        .texture("layer0",
                                new ResourceLocation(MOD_ID, "item/" + itemDeferredHolder.getId().getPath()));
            });
        }
    }
    ...
    private ItemModelBuilder handheldItem(DeferredHolder<Item, ? extends Item> item) {
        return withExistingParent(item.getId().getPath(),
                ...
                new ResourceLocation(UsefultoolsMod.MOD_ID, "item/" + item.getId().getPath()));
    }

    public void buttonItem(DeferredHolder<Block, ? extends Block> block, DeferredHolder<Block, Block> baseBlock) {
        this.withExistingParent(BuiltInRegistries.BLOCK.getKey(block.get()).getPath(), mcLoc("block/button_inventory"))
                .texture("texture",  new ResourceLocation(UsefultoolsMod.MOD_ID, "block/" + BuiltInRegistries.BLOCK.getKey(baseBlock.get()).getPath()));
    }

    public void fenceItem(DeferredHolder<Block, ? extends Block> block, DeferredHolder<Block, Block> baseBlock) {
        this.withExistingParent(BuiltInRegistries.BLOCK.getKey(block.get()).getPath(), mcLoc("block/fence_inventory"))
                .texture("texture",  new ResourceLocation(UsefultoolsMod.MOD_ID, "block/" + BuiltInRegistries.BLOCK.getKey(baseBlock.get()).getPath()));
    }

    public void wallItem(DeferredHolder<Block, ? extends Block> block, DeferredHolder<Block, Block> baseBlock) {
        this.withExistingParent(BuiltInRegistries.BLOCK.getKey(block.get()).getPath(), mcLoc("block/wall_inventory"))
                .texture("wall",  new ResourceLocation(UsefultoolsMod.MOD_ID, "block/" + BuiltInRegistries.BLOCK.getKey(baseBlock.get()).getPath()));
    }

...
public class ModEvents {
    ...
    private static void handleCoalToolBurning(Player player) {
        ...
        if (player.tickCount % 20 == 0) {
            ...
            if (mainBurning) {
                main.hurtAndBreak(2, player, e -> e.broadcastBreakEvent(EquipmentSlot.MAINHAND));
            ...
            if (offBurning) {
                off.hurtAndBreak(2, player, e -> e.broadcastBreakEvent(EquipmentSlot.OFFHAND));
        ...
        if (player.tickCount % 20 == 0) {
            ...
            for (EquipmentSlot slot : ARMOR_SLOTS) {
                ...
                if (isCoalArmor(piece) && CoalBurningHelper.isBurning(piece)) {
                    piece.hurtAndBreak(1, player, e -> e.broadcastBreakEvent(slot));
                }
            }
        }
    ...
    private static void handleSnowMelt(Player player) {
        ...
        if (player.tickCount % 40 == 0) {
            ...
            if (isSnowTool(main)) { main.hurtAndBreak(1, player, e -> e.broadcastBreakEvent(EquipmentSlot.MAINHAND)); melted = true; }
            if (isSnowTool(off))  { off.hurtAndBreak(1, player, e -> e.broadcastBreakEvent(EquipmentSlot.OFFHAND));   melted = true; }
    ...
    private static void handleSnowFireProtection(Player player) {
        ...
        if (player.tickCount % 20 == 0) {
            if (isSnowTool(main)) main.hurtAndBreak(5, player, e -> e.broadcastBreakEvent(EquipmentSlot.MAINHAND));
            if (isSnowTool(off))  off.hurtAndBreak(5,  player, e -> e.broadcastBreakEvent(EquipmentSlot.OFFHAND));
        }
    ...
    private static void handleIceMelt(Player player) {
        ...
        if (player.tickCount % 60 == 0) {
            ...
            ItemStack off  = player.getOffhandItem();
            boolean melted = false;
            if (isIceTool(main)) { main.hurtAndBreak(1, player, e -> e.broadcastBreakEvent(EquipmentSlot.MAINHAND)); melted = true; }
            if (isIceTool(off))  { off.hurtAndBreak(1, player, e -> e.broadcastBreakEvent(EquipmentSlot.OFFHAND));   melted = true; }
    ...
    private static void handleIceFireProtection(Player player) {
        ...
        if (player.tickCount % 20 == 0) {
            if (isIceTool(main)) main.hurtAndBreak(5, player, e -> e.broadcastBreakEvent(EquipmentSlot.MAINHAND));
            if (isIceTool(off))  off.hurtAndBreak(5,  player, e -> e.broadcastBreakEvent(EquipmentSlot.OFFHAND));
            for (EquipmentSlot slot : ARMOR_SLOTS) {
                ItemStack piece = player.getItemBySlot(slot);
                if (isIceArmor(piece)) piece.hurtAndBreak(5, player, e -> e.broadcastBreakEvent(slot));
            }
        }
    ...
    private static boolean isIceArmor(ItemStack stack) {
        ...
        return ModArmorMaterials.ICE == armor.getMaterial();
    }
    ...
    private static boolean isPprismArmor(ItemStack stack) {
        if (stack.isEmpty() || !(stack.getItem() instanceof ArmorItem armor)) return false;
        return ModArmorMaterials.PPRISM == armor.getMaterial();
    }
    ...
    private static boolean isOpArmor(ItemStack stack) {
        if (stack.isEmpty() || !(stack.getItem() instanceof ArmorItem armor)) return false;
        return ModArmorMaterials.OVERPOWER == armor.getMaterial();
    }

    private static boolean isFniArmor(ItemStack stack) {
        if (stack.isEmpty() || !(stack.getItem() instanceof ArmorItem armor)) return false;
        return ModArmorMaterials.FNI == armor.getMaterial();
    }

    private static boolean isEctoArmor(ItemStack stack) {
        if (stack.isEmpty() || !(stack.getItem() instanceof ArmorItem armor)) return false;
        return ModArmorMaterials.ECTO == armor.getMaterial();
    }
    ...
    private static void handleFniBootsFire(Player player) {
        ...
        if (floorState.getFlammability(level, feetPos.below(), Direction.UP) > 0
    ...
    private static void handleFoodHungerDrain(Player player) {
        ...
        if (isFoodTool(main) && main.isDamageableItem() && isFoodSetEnabled(main)) {
            main.hurtAndBreak(1, player, e -> e.broadcastBreakEvent(EquipmentSlot.MAINHAND));
            player.getFoodData().eat(1, 0.1f);
            spawnFoodParticles(player, serverLevel);
        }
        if (isFoodTool(off) && off.isDamageableItem() && isFoodSetEnabled(off)) {
            off.hurtAndBreak(1, player, e -> e.broadcastBreakEvent(EquipmentSlot.OFFHAND));
            player.getFoodData().eat(1, 0.1f);
            spawnFoodParticles(player, serverLevel);
        }
    ...
    private static boolean isCakeArmor(ItemStack stack) {
        if (stack.isEmpty() || !(stack.getItem() instanceof ArmorItem armor)) return false;
        return ModArmorMaterials.CAKE == armor.getMaterial();
    }
    ...
    private static boolean isBreadArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.BREAD == a.getMaterial();
    }
    ...
    private static boolean isDriedKelpArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.DRIED_KELP == a.getMaterial();
    }
    ...
    private static boolean isRottenFleshArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.ROTTEN_FLESH == a.getMaterial();
    }
    ...
    private static boolean isMelonArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.MELON == a.getMaterial();
    }
    ...
    private static boolean isSweetBerryArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.SWEET_BERRY == a.getMaterial();
    }
    ...
    private static boolean isPumpkinPieArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.PUMPKIN_PIE == a.getMaterial();
    }
    ...
    private static boolean isMushroomArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.MUSHROOM == a.getMaterial();
    }
    ...
    private static boolean isPufferfishArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.PUFFERFISH == a.getMaterial();
    }
    ...
    private static boolean isHoneyArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.HONEY == a.getMaterial();
    }
    ...
    private static boolean isChorusFruitArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.CHORUS_FRUIT == a.getMaterial();
    }
    ...
    private static boolean isGoldenAppleArmor(ItemStack s) {
        return !s.isEmpty() && s.getItem() instanceof ArmorItem a && ModArmorMaterials.GOLDEN_APPLE == a.getMaterial();
    }
        ...
        if (source.getEntity() instanceof Player attacker) {
            ...
            if (Config.paperEnabled && Config.paperEffects && isPaperTool(held)) {
                ...
                if (attacker.level().random.nextFloat() < 0.25f)
                    held.hurtAndBreak(held.getMaxDamage() - held.getDamageValue(), attacker, e -> e.broadcastBreakEvent(EquipmentSlot.MAINHAND));
            }
    ...
    private static boolean isCactusArmor(ItemStack s) {
        ...
        return ModArmorMaterials.CACTUS == a.getMaterial();
    }
    ...
    private static boolean isBoneArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.BONE == a.getMaterial();
    }
    private static boolean isClayArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.CLAY == a.getMaterial();
    }
    ...
    private static boolean isNetherBrickArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.NETHER_BRICK == a.getMaterial();
    }
    private static boolean isCopperArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.COPPER == a.getMaterial();
    }
    ...
    private static boolean isPhantomArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.PHANTOM == a.getMaterial();
    }
    ...
    private static boolean isMagmaCreamArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.MAGMA_CREAM == a.getMaterial();
    }
    ...
    private static boolean isSlimeArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.SLIME == a.getMaterial();
    }
    ...
    private static boolean isBlazeArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.BLAZE == a.getMaterial();
    }
    ...
    private static boolean isNautilusArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.NAUTILUS == a.getMaterial();
    }
    ...
    private static boolean isPurpurArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.PURPUR == a.getMaterial();
    }
    ...
    private static boolean isGhastTearArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.GHAST_TEAR == a.getMaterial();
    }
    ...
    private static boolean isEyeOfEnderArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.EYE_OF_ENDER == a.getMaterial();
    }
    ...
    private static boolean isShulkerArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.SHULKER == a.getMaterial();
    }
    private static boolean isTurtleScuteArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.TURTLE_SCUTE == a.getMaterial();
    }
    ...
    private static boolean isEchoShardArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.ECHO_SHARD == a.getMaterial();
    }
    ...
    private static boolean isDragonBreathArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.DRAGON_BREATH == a.getMaterial();
    }
    private static boolean isRabbitHideArmor(ItemStack s) {
        if (s.isEmpty() || !(s.getItem() instanceof ArmorItem a)) return false;
        return ModArmorMaterials.RABBIT_HIDE == a.getMaterial();
    }

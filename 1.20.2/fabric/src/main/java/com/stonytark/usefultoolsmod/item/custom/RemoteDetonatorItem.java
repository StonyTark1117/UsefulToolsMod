...
public class RemoteDetonatorItem extends Item {
    ...
    @Override public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        NbtList links = stack.getOrCreateNbt().getList(LINKS, NbtElement.COMPOUND_TYPE);
        ...
        if (!exists && links.size() < 16) { NbtCompound link = new NbtCompound(); link.putLong("Pos", pos.asLong());
            link.putString("Dimension", dimension); links.add(link); stack.getOrCreateNbt().put(LINKS, links); }
    ...
    @Override public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (player.isSneaking()) { int next = (channel(stack) + 1) % 8; stack.getOrCreateNbt().putInt(CHANNEL, next);
    ...
    private static int channel(ItemStack stack) { return Math.floorMod(stack.getOrCreateNbt().getInt(CHANNEL), 8); }
    private static void detonate(ServerPlayerEntity player, ItemStack stack) {
        NbtList oldLinks = stack.getOrCreateNbt().getList(LINKS, NbtElement.COMPOUND_TYPE); NbtList retained = new NbtList();
        ...
        stack.getOrCreateNbt().put(LINKS, retained);

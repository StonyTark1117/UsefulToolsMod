...
public class WraithEntity extends HostileEntity {
    ...
    @Override protected void dropLoot(DamageSource source, boolean causedByPlayer) {
        ...
        if (source.getAttacker() instanceof PlayerEntity player) {
            var enchantment = getWorld().getRegistryManager().get(net.minecraft.registry.RegistryKeys.ENCHANTMENT)
                    .getEntry(net.minecraft.enchantment.Enchantments.LOOTING).orElseThrow();
            looting = net.minecraft.enchantment.EnchantmentHelper.getLevel(enchantment, player.getMainHandStack());
        }

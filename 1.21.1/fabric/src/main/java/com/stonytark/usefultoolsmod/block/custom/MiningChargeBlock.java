...
public class MiningChargeBlock extends BlockWithEntity {
    public static final MapCodec<MiningChargeBlock> CODEC = createCodec(MiningChargeBlock::new);
    ...
    @Override protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos,
                                                       PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!Config.miningChargeEnabled) return ItemActionResult.FAIL;
        if (stack.getItem() instanceof FlintAndSteelItem) {
            if (!world.isClient && player instanceof ServerPlayerEntity serverPlayer
                    && world.getBlockEntity(pos) instanceof MiningChargeBlockEntity charge) {
                charge.prime(serverPlayer);

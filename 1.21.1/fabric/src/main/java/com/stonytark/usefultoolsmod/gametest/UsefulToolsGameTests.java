...
public final class UsefulToolsGameTests implements FabricGameTest {
    ...
    public void registrationsAndRecipeData(TestContext context) {
        ...
        context.assertTrue(ModItems.RGOLD_PICKAXE.getDefaultStack().getMaxDamage()
                        > Items.IRON_PICKAXE.getDefaultStack().getMaxDamage(),
                "ferrous-gold tier must outlast iron");
        context.assertTrue(context.getWorld().getRegistryManager().get(RegistryKeys.PLACED_FEATURE)
                        .containsId(Identifier.of(UsefultoolsMod.MOD_ID, "rgold_ore_placed")),
    ...
    public void spectralInfuserProcesses(TestContext context) {
        ...
        infuser.setStack(1, new ItemStack(ModItems.ECTOPLASM));
        context.runAtTick(205, () -> {
            context.assertTrue(infuser.getStack(0).isEmpty(), "input must be consumed");
            context.assertTrue(infuser.getStack(1).isEmpty(), "one ectoplasm must be consumed");
            context.assertTrue(infuser.getStack(2).isOf(Items.IRON_PICKAXE), "tool must reach output");
            context.assertTrue(EctoplasmInfusionHelper.isInfused(infuser.getStack(2)), "output must be infused");
            infuser.setStack(2, ItemStack.EMPTY);
            infuser.setStack(0, new ItemStack(Items.IRON_PICKAXE));
            infuser.setStack(1, new ItemStack(ModItems.ECTOPLASM));
            ...
            context.assertTrue(infuser.getStack(0).isOf(Items.IRON_PICKAXE), "disabled Infuser must retain input");
            context.assertTrue(infuser.getStack(1).isOf(ModItems.ECTOPLASM), "disabled Infuser must retain fuel");
            context.complete();
        });
    }
    ...
    public void ghostAndDamageContracts(TestContext context) {
        ...
        context.assertTrue(EctoplasmArmorHelper.isGhostInvisible(player),
    ...
    private static void equipInfusedArmor(PlayerEntity player, EquipmentSlot slot, Item item) {
